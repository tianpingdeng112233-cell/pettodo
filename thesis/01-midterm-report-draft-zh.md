...
截稿时,核心循环在双端完整可用,应用、生成管线与孵化后端三套测试共 112 个全部通过。期中之后,项目又交付了
...
**图 9 — Little Theater(小剧场),当日清单全部完成时出现。**
...
`flutter test` 在主线通过 58 个测试(20 个文件),加上骨架渲染器的新增后是 65
个。生成管线有自己的套件:25 个测试、6 个文件;孵化后端另有 22 个、4 个文件——
以上每个数字都是现场跑出来的,不是引用来的。未合入的通知
