...
孩子、宠物、一个依赖你的人——常常可以。所以 Pawside 不发提醒,而是给用户一只由
自家动物照片生成的虚拟宠物,把一张小小的每日任务清单变成"我们一起做的事"。
...
以及围绕关系重做的 onboarding。剩余工作是一场"检验机制、而非假设机制"的评估。
...
被弃用的内疚。目标:(i) 双端核心循环完全离线运行,无账号、无服务器;(ii) 宠物由
用户*自家*动物生成,假设是"对真实、可辨认的同伴的依恋,强于对通用形象的依恋";
...
我用为本报告收集的 4,754 条商店评论(方法与采样口径见 §2.2)、公开商店元数据,
...
十六个问题做了一次结构化文献检索来核查这两件事(方法见 §2.2)。结果两边的立场
...
改由 §1.3.2 的有效范围观察来辩护:小的日常维护,是宠物机制唯一起作用的地方。
...
现有交互选择里证据基础最好的一个,依据就是 §1.1 的前瞻记忆分离。值得一提的是,
...
方法论:这让评估不依赖数据基础设施就能进行。§5 记录了代价:留存没法按任何被引
研究的方式测量。项目现在唯一的服务器,孵化代理(§3.5),不保存用户数据:它存在
...
| 动画来源 | v3 rig pack:4 张姿势+骨架 | v2 逐帧生成 | 都能生成可辨认的宠物;v2 每只约 40 分钟且帧间漂移,v3 约 1 分钟且动作确定(§3.5) |
| 视觉风格 | 像素风("Cozy Pixel") | 照片衍生的柔和风 | 柔和风温度好但撑不住稳定剪影(§4.4);像素稳定,还回应了"幼稚"的指控 |
| 通知线索 | 事件触发(规划中,§6) | 按时钟触发 | ADHD 成年人的时间触发前瞻记忆受损,事件触发被豁免 |
...
**养成系统。**状态 schema v2 带就地迁移,成长阶段由推导得出而非落盘,零食经济、
全天宠物作息、长按互动,以及一个没有进度条的收藏馆。

**待办基线。**Schema v3。任务成为按 ID 寻址的对象,类型是 `daily` 或 `oneOff`,
...
v2 管线(§3.2)靠"每一帧都单独生成一张图"来产出宠物:约 14 次串行生成、每只
约 40 分钟,帧间漂移见 §3.2,成品包还要人工发回给用户。这条路线已经退役。v3
管线只生成四张正典姿势——坐姿睁眼、同姿闭眼、蜷卧睡姿、侧视——外加头、尾、腿
...
key、执行物种门(v1 只收猫狗)、每账号孵化配额、限流和成本封顶——然后把成品包
直接送回设备。不建账号,服务端不留照片。存量 v2 宠物照常工作:渲染器按包格式
路由,两种格式并行。早期动画草案里的"失败"反应类,按红线永久移除。

**图 4 — 两代孵化链路。**v2 逐帧生成;v3 只生成四张姿势,动起来的事交给骨架。
![v2 与 v3 孵化管线对比流程图](figures/hatch-pipeline.png)
...
原版 onboarding 在介绍功能。重做后的流程先介绍宠物,依据正是 §1.3.2:应用是
为了看宠物才被打开的,所以第一分钟应该用来建立这层联结。用户先见到一格领养
网格("Who's coming home?"),挑一只,给它起名——旁边有个骰子按钮,从预置名池
里随机换一个,让唯一需要打字的时刻有一条零成本退路。然后从可点选的 chips 里挑
最多三件"小事"(起床、喝点水、吃药……),只有自定义条目才需要键盘。中场庆祝
在任何真实任务完成*之前*就发出第一颗零食——奖励循环靠演示,不靠说明。Android
上,§3.4 的悬浮层以邀请的形式出现("Can I stay on your screen?"),走同一套
...
已实现但不合入,因为文献与其三个机制中的两个相抵触。§6 解释冲突与解决路径。
...
遇到它们的顺序排列。它们记录的是期中时点、像素改版(§3.3)之前的构建:布局、
文案与行为是当前的,视觉风格此后已更换。
...
——§3.2 的运行时注册表在这里变得肉眼可见,差异化功能也在这里第一次变得可触摸。
...
一起做掉的原因:像素让帧稳定(§3.3),骨架让运动确定(§3.5)。这个审美选择还
顺带回应了 §1.3.2 里记录在品类第一名身上的"幼稚"指控。
...
覆盖集中在产品对用户的承诺上:schema v1→v2→v3 迁移;日翻转,包括连续漏掉多日
...
完整方案另有专门文档。概要:五到十名种子用户,ADHD 倾向的宠物主人,只经我个人
网络招募;为期十四天;第 7、14 天各一份简短问卷;用户主动发起的本地数据导出是
唯一数据通道;对内疚、以及对自家动物的"恐怖谷"反应做定性红灯监测,任何一例都
比留存数字更重要。在 ADHD 社区隐蔽招募被排除,既出于伦理也出于现实——社区调研
记录过版主公开点名搞隐形营销的产品。伦理审批是周期最长的一项,也是要问学校的
第一个问题。

由文献引出的两条评估修订记录在此,因为它们改变评估能主张什么。第一,社区口径
">50% 两周留存"混淆了两个不可比的框架:心理健康类应用真实世界的 15 天留存
中位数是 3.9%([Baumel et al. 2019](https://doi.org/10.2196/14567)),而招募式
试验能到 75% 上下,且报告的使用量比同一批程序的真实世界使用高中位 4.06 倍
([Baumel et al. 2019](https://doi.org/10.1093/tbm/ibz147))。招募来的种子组就是
试验人群,>50% 在这个框架里是低标准、在另一个框架里是奇迹——报告会写明自己在
哪个框架里。第二,两周本来就是错误的窗口:评论语料定位了两个失败点,第 2–4 天
(布置热情消退)和第二个月前后(新鲜感消退)。因此增加第 60 天检查点。未设盲的
自我报告还是这个领域最容易膨胀的指标——儿童行为干预的效应在大概率设盲的评估下
塌缩为不显著
([Sonuga-Barke et al. 2013](https://doi.org/10.1176/appi.ajp.2012.12070991))——
所以分析在问卷与事件日志冲突时,一律以日志为准。
...
**收尾 v3。**管线、代理、渲染器都在了(§3.5);剩下的是生产与产品化:用同一条
管线生成一批预置宠物,让领养网格有真实的可选性;自家宠物孵化的一次性解锁流;
以及 iOS 小组件作为第二个常驻表面——它现在可以直接复用 Android 悬浮层所画的
像素素材形态。

**评估。**伦理问询、招募、十四天试用加第 60 天回访、分析。悬而未决的智力工作是:
当核心机制在现有证据下尚未被证明时,项目到底主张什么——并把评估做得足够诚实,
让它能给出答案。
