...
截稿时,核心循环在双端完整可用,应用、生成管线与孵化后端三套测试共 156 个全部
通过。期中之后,项目又交付了像素风改版、Android 常驻宠物悬浮层、一条约一分钟把
照片变成会动宠物的生成管线、围绕关系重做的 onboarding,以及一层重写到"宠物只谈
自己的一天"的通知。能检验锚点机制的用户研究已经设计就绪,但塞不进项目窗口;报告对"没有它,什么
能主张、什么不能"写得清清楚楚。
...
没有任何提醒从那里发出,它只是陪着。它对应用内发生的事只有单向反应——在应用里
完成一个任务,悬浮宠一秒内开始庆祝;任何负面的东西都不会传到它那里。宠物位置
可拖动,连应用进程被杀都记得住。用户拒绝悬浮层权限,应用就静默继续,并且永不
再问。目前悬浮层画的是内置宠物;把用户领养的宠物接上去是排队中的后续工作
(第 6 节)。

**图 4 — 住进用户屏幕的宠物。**左:悬浮宠在 Android 桌面上,和用户自己的图标
待在一起。右:同一只宠物浮在另一个应用之上。
![两张 Android 截图:像素宠物浮在桌面上与 Chrome 之上](figures/app/08-android-overlay.png)
...
画风。图 5 对比两条路线。现在孵化一只约一分钟,模型费用约 £0.03(¥0.3),这让
应用内直接孵化变得可行:照片从应用发到一个小型代理后端,由它持有图像模型的
key、执行物种门(目前只收猫狗)、按设备计的孵化配额、限流和成本封顶——然后把成品包
...
的照片会被温柔谢绝,并把用户想要的物种登记为未来模板的愿望。把做好的九只包
内置进应用是排队中的下一步;领养网格(第 3.6 节)渲染实时注册表,包一落地就会
自动填满。

**图 5 — 两代孵化链路。**第一代逐帧生成;第二代只生成四张姿势,动起来的事交给骨架。
![两代孵化管线对比流程图](figures/hatch-pipeline.png)

管线是一个 Python CLI(`tools/rig_pipeline/`,27 个测试),内建重试与二选一采样,
...
一个小实验敲定了照片输入的设计,见图 6:同一张正典姿势,分别用这只猫的一张照片
和三张照片生成,各出四张候选。单张照片下,候选之间的身份已经稳定——多给照片的
收益不是稳定性,而是*覆盖面*:相机没拍到的那些部位,比如玳瑁猫两侧不对称的
花色,单张照片下是被模型合理地编出来的,三张照片下才是照实画出来的。所以输入
收一到三张照片,应用内的文案引导"补不同角度",而不是"多传几张"。

**图 6 — 一张照片对三张照片。**上排:单张照片生成的四张候选。下排:同一只猫
三张照片生成的四张候选。身份两边都稳;多出来的照片修正的是单张照片从未拍到的
那一侧花色。
![八只生成的像素猫,对比单照片与三照片输入](figures/multi-photo-test.png)

### 3.6 Onboarding:围绕关系重做

原版 onboarding 在介绍功能。重做后的流程先介绍宠物,依据正是第 1.3.2 节:应用是
为了看宠物才被打开的,所以第一分钟应该用来建立这层联结。用户先见到领养之问
——"Who's coming home?"——下面是一格从实时宠物注册表渲染的网格(永不写死;
第 3.5 节的九只名册随包内置逐步填满),挑一只,
...
通知权限也顺延到那个时刻才索取。老用户不会重走 onboarding。图 7 是全新安装下
这条流程的四个瞬间。

**图 7 — Onboarding 四瞬间。**从左到右:领养之问,自家宠物解锁只占安静的一行;
三件小事做成可点选的 chips,全程不需要打字;中场庆祝在任何任务存在之前就发出
第一颗零食;落地 Home,预埋的首胜任务——"Give Choco a pat — a free one, to
see how it feels"(摸摸 Choco——白送的,感受一下)——等在清单最上方。
![四张 onboarding 截图:领养、chips、庆祝、首胜](figures/app/09-onboarding.png)

### 3.7 通知层:冲突解决,已上线

期中时点,通知层的一次重做已实现却被刻意压着不合入,因为文献与其三个机制中的
两个相抵触。这个冲突后来解决了——靠的不是代码,而是一条写进项目宪法的裁定:
**宠物的幸福永远不是用户在场与否的函数。**宠物过着自己的好日子;它可以开口,
但只谈自己——在场,但不索取。在这个语义上,重做合入上线。整个消息池现在都是
宠物在汇报自己的一天(晒到一束阳光、窗台来了只鸟、伸了个大懒腰);"等你回来/
想你了/别忘了"这类措辞在屏幕两侧一并封禁,由一条构建即败的 lint 强制;触发
时间保留 ±10 分钟抖动;连续未打开则频率退避——每天、隔天、每周——一打开应用
即复位。文献最尖锐的反对(第 6 节)是:主动退避是模糊的疏远,而拒绝敏感会把
模糊读成拒绝;在"自己过得很好"的语义下,这个模糊消失了——一只从不等待的宠物
没法被读成"放弃了你",只是它的近况来得少了。应用内两句违反此规的旧文案
("Just because I miss you")也在同一改动里重写。
...
以下截图出自 Android 模拟器上一次连续会话,跑的是现行 release 构建,与图 7
同一次全新安装——onboarding 里领养了预置宠物 Choco,首胜任务已完成。顺序按
用户遇到它们的顺序排列。
...
动画,掉下一颗零食——计数跳了一格。屏幕上没有分数、没有连击、没有进度条、
没有"完成 3/4",因为进度指示器同时也是缺口指示器。零食是唯一货币,用途是喂
宠物,而且花不花随意。

**图 11 — Little Theater(小剧场),当日清单全部完成时出现。**
...
历史页是安全架构最清晰的一次表达。它写着 "This week, you and Choco did 4
things together"(这周你和 Choco 一起做了 4 件事)——第四件正是预埋的首胜
任务——而且只列出周三,因为周三是唯一有完成记录的日子。没有完成记录的日子不是被画成空白、灰色或零:**它们
...
一个控件即可删除;没有归档、没有已完成列表、没有任何让做完的事堆积的地方。
第 3.4 节的悬浮层在这里是一行开关——"Let Choco stay on your screen"。通知区
就是一个开关加三个固定时间,而它的副标题正是第 3.7 节的宪法上屏:"All quiet —
Choco is happily minding its own day"(一切安好——Choco 正快活地过自己的
日子)。通知默认关闭,直到用户明确打开;权限一旦被拒,永不再问。

### 4.2 孵化环,端到端

下面这轮验收走查走的是第一代流程——靠导出请求档、再手工导入成品包;换代后的
管线(第 3.5 节)这两步都不需要了,但走查记录保留,因为它是真正在设备上端到端
验证过的东西。走查在同一台设备上从全新安装开始:onboarding → 孵化页 → 拍照
...
第 3 节描述的每一波都合入之后,`flutter test` 通过 107 个测试(27 个文件)。
生成管线有自己的套件:27 个测试、6 个文件;孵化后端另有 22 个、4 个文件——
以上每个数字都是现场跑出来的,不是引用来的。套件里包含那条 lint:通知文案若
重新出现违禁措辞,构建直接失败(第 3.7 节)。
...
**通知冲突,留档在案。**当初压住重做的证据,即便第 3.7 节已经上线,也值得留在
视野里:最接近的检验发现,精心构建的 30 句消息库不比一句固定文案更好
([Bell et al. 2023](https://doi.org/10.2196/38342));时间抖动与习惯养成的线索
一致性论相抵触([Lally et al. 2010](https://doi.org/10.1002/ejsp.674));一项
试验发现*更低频*的通知带来更少的查看与执行
([Morrison et al. 2017](https://doi.org/10.1371/journal.pone.0169162));而主动
减少联系在构造上就是模糊行为,拒绝敏感会把模糊读成拒绝
([Downey & Feldman 1996](https://doi.org/10.1037/0022-3514.70.6.1327))。宪法
式的解法——宠物永远只谈自己的一天——是一个论证:危险的是模糊,不是退避本身。
这是一个设计判断,不是一个实证发现;它成不成立,正是提交后那场用户研究要盯着
看的东西。
...
**把回归画面与明确的暂停做出来。**零惩罚挡不住积压的清单自己开口——*羞耻
提醒器*。语义已由宪法定下(第 3.7 节):缺席期间宠物一直过着自己的好日子,
所以归来第一屏展示的是宠物积累的新鲜事,不是用户的欠账。把这块屏幕、连同一个
把缺席改写为"选择"而非"失败"的明确暂停动作做出来,是下一件内疚工程。

**把导出升级为用户可见的备份。**应用只存本地,宠物由用户自家动物生成、因此不可
替代,而数据丢失是品类第一名得票最高的投诉——它至少还有账号可恢复。导出机制已
存在;把它变成一个带诚实说明的可见备份,是一个小改动,对冲的是不成比例的风险。

**收尾新管线。**管线、代理、渲染器、预置名册和解锁流都在了(第 3.5 节);剩下
的全是运营件:把九只预置包内置进应用让领养网格填满、部署代理后端并把应用指过
去、给解锁定价(目前带的是占位价)、把领养的宠物接上 Android 悬浮层,以及 iOS
