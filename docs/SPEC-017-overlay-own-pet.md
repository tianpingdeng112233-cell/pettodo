...
## 修订(⚖️2026-08-28 验收返修轮)

- **pixelScale=2 定为正典**:旧值 1 与 IMPLEMENTATION-NOTES 的「整数 2×」及 defaultX 的 2× 定位自相矛盾,模拟器实测 2× 尺寸正确。
- **四耳 bug**:Gemini head 框可能只框住耳朵不含脸(实证 pack 922de144)。后端加 head 框几何质量门+一次重试,仍无效则出包但 head 框置空;客户端加载时同口径校验,无效则退化为整图微动+全图眨眼(存量已孵宠自动自愈,不重孵)。
- **白斑 bug**:768px 源图 nearest 直缩到 192 画布把浅色高光采到轮廓上;共享渲染管线先做一次高质量预缩到目标分辨率,后续保持 nearest/整数。验收:烘焙帧深底扫描浅色杂点 <5。
- 互审修复:换宠烘焙加 epoch 守卫;气泡时点亮屏时按墙钟校准(过期 >2 分钟丢弃);悬浮层关闭时不烘、同包缓存复用、idle 帧上限 32;帧路径未变不重发不重解码;10.0.2.2 默认仅限 Android debug。
