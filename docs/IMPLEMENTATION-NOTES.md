...
- Notification scheduling keeps the optional evening invitation and enabled recurring task times in their existing shared, time-sorted 32-slot window. Future concrete-date task reminders are appended in separate capacity (up to seven, for a maximum of 39 scheduled entries) so they never displace legacy recurring notifications. Each recurring source contributes at most once per local day; a completed daily skips its current-day reminder, one-off completion removes its future concrete-date reminder, and launch/resume/settings changes refill the schedule. The persisted global permission state is shared, so a denial from either opt-in prevents every later flow from requesting again.
