...
## 已知限制（⚖️2026-08-30 David 拍板接受）

Android 通知插件的 boot receiver 会在重启后恢复缓存的定时通知：设备**关机跨过提醒时刻后
开机的瞬间**，该条一次性提醒会被系统补发一次（迟到弹出）。app 每次打开时全量 cancel+重排、
只排未来项，故其余路径均严格「错过静默」；此重启窗口作为已知限制接受，不引入耦合插件
内部存储的原生拦截（维护成本不成比例）。
